...
function LoginForm(handle: Handle, setup: LoginFormSetup = {}) {
	...
	return () => {
		...
		const description = isSignup
			? 'Sign up to start using epic-agent.'
			: 'Log in to continue to epic-agent.'
...
function ResetPasswordForm(handle: Handle) {
			...
			>
				<header css={{ display: 'grid', gap: spacing.xs }}>
					...
					<p css={{ color: colors.textMuted }}>
						You are signed in to epic-agent.
					</p>
				</header>
				{status === 'loading' ? (
		...
		return (
			...
			>
				<header css={{ display: 'grid', gap: spacing.xs }}>
					...
					<p css={{ color: colors.textMuted }}>
						{clientLabel} wants to access your epic-agent account.
					</p>
				</header>
				<section
					css={{
